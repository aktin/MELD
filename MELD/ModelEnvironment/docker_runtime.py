...
def stream_container_logs(container: Container, job_context: ExecutionContext) -> None:
    """
    ...
    Parameters:
        ...
        job_context (ExecutionContext): The job context holding the logger for
    ...
    """
    try:
        container_logger = get_inference_logger(job_context.execution_id)
...
def pull_image(
    ...
    try:
        ...
        if progress_callback is None:
            if auth_config:
                _client().images.pull(image, auth_config=auth_config)
            else:
            ...
            for event in _client().api.pull(image, **pull_kwargs):
                if "error" in event:
...
def ensure_image_exists(job_context: ExecutionContext) -> None:
    """
...
def start_container(container: Container, job_context: ExecutionContext, ) -> None:
    """
    ...
    Parameters:
        ...
        job_context (ExecutionContext): The context of the job, including logging and
            status tracking.
    ...
    try:
        ...
        job_context.set_status(ExecutionStatus.RUNNING)
    except APIError as e:
...
def wait_for_container(container: Container, job_context: ExecutionContext, timeout_seconds: int = 500) -> int:
    """
    ...
    Parameters:
        ...
        job_context (ExecutionContext): The context of the job being executed, used for logging and status tracking.
    ...
    try:
        ...
        if exit_code != 0:
            ...
            job_context.set_status(ExecutionStatus.SUCCESS)
    except requests.exceptions.ReadTimeout:
        error = f"Runtime container timed out after {timeout_seconds} seconds"
        job_context.logger.error(error)
        job_context.set_status(ExecutionStatus.TIMEOUT)
        container.kill()
...
def stop_container(container: Container, job_context: ExecutionContext):
    """
    Stops the given runtime container.

    Args:
        container (Container): The runtime container to be stopped.
        job_context (ExecutionContext): The context of the job, providing access to
...
def destroy_container(container: Container, job_context: ExecutionContext) -> None:
    """
    Destroys a specified container and logs the operation.

    Args:
        container (Container): The container object that needs to be destroyed.
        job_context (ExecutionContext): The context of the job, which includes logging.
    """
...
def create_container(image: str, job_context: ExecutionContext, ) -> Container:
    """
    ...
    try:
        ...
        runtime_container = _client().containers.create(image,
                                                     environment=environment_variables,
                                                     name=f"runtime_{job_context.contract.id}_{job_context.execution_id}",
                                                     )
        job_context.logger.info(f"Created runtime container {runtime_container.name}")
        job_context.set_status(ExecutionStatus.CREATED)
...
def get_image_size(image: str, job_context: ExecutionContext, ):
    try:
        docker_image = _client().images.get(image)
