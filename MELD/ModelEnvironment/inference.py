...
from ModelEnvironment.docker_runtime import create_container, start_container, wait_for_container, \
    stop_container, destroy_container, get_image_size
...
class InferenceRunner:
    ...
    def run(self) -> str | None:
        ...
        """
        archive_bytes = None
        self.journal.append_entry("inference_started", "Starting inference runtime")
        self.monitor.update_metric_value(Metrics.INFERENCE_INPUT_ROW_COUNT, len(self.input_data.index))
        try:
            self.monitor.update_metric_value(
                Metrics.DOCKER_IMAGE_SIZE,
                get_image_size(self.image, self.job_context, self.journal),
            )

            self.runtime_container = create_container(self.image, self.job_context, self.journal)

            self.create_interface_folders()
            self.copy_data_to_container()

            self.monitor.start_inference_time()
            start_container(self.runtime_container, self.job_context, self.journal)

            exit_code = wait_for_container(self.runtime_container, self.job_context, self.journal)

            stop_container(self.runtime_container, self.job_context, self.journal)
            ...
            self.journal.append_entry("output_collection_started", "Collecting runtime output")
            ...
            self.journal.append_entry(
                ...
                result_row_count=len(output_df.index),
                result_column_count=len(output_df.columns),
            )
            self.journal.append_entry("job_completed", "Inference job completed successfully")

        except Exception as error:
            self.journal.append_entry("inference_failed", str(error), error_type=type(error).__name__)
            self.journal.append_entry("job_failed", "Inference job failed")
            self.job_context.logger.exception(f"An exception occurred during inference: {error}")
        finally:
            self.pack_archive(archive_bytes)
            self._destroy_runtime_container()

            return self.output_zip_path

    def _destroy_runtime_container(self) -> None:
        if self.runtime_container is None:
            return

        try:
            destroy_container(self.runtime_container, self.job_context, self.journal)
        except Exception as error:
            self.journal.append_entry(
            ...
            self.job_context.logger.exception(f"Failed to destroy runtime container: {error}")


    def pack_archive(self, archive_bytes: bytes | None):
        self.journal.append_entry("archive_started", "Creating execution archive")
    ...
    def create_interface_folders(self) -> None:
        ...
        try:
            ...
            self.journal.append_entry("runtime_interface_created", "Created runtime input and output folders")
        except Exception as e:
            error = "Failed to create interface folders"
            self.journal.append_entry("runtime_interface_creation_failed", error, error_detail=str(e))
    ...
    def copy_data_to_container(self) -> None:
        ...
        try:
            ...
            timespan = self.monitor.stop_input_data_copy_time()
            self.journal.append_entry(
                "input_copied",
                "Copied input data and contract into runtime container",
                size_bytes=len(input_csv.encode("utf-8")),
                duration_seconds=timespan.total_seconds(),
            )
        except APIError as e:
            error = "Failed to copy input data into runtime container"
            self.journal.append_entry("input_copy_failed", error, error_detail=str(e))
            raise RuntimeError(error) from e
        except Exception as e:
            error = "Failed to copy input data into runtime container"
            self.journal.append_entry("input_copy_failed", error, error_detail=str(e))
    ...
    def extract_result_file(self, archive_bytes: io.BytesIO) -> bytes:
        ...
        if extracted is None:
            error = "Result file output.csv not found in inference runtime output folder"
            self.journal.append_entry("output_missing", error)
        ...
        self.journal.append_entry(
            "output_extracted",
            "Extracted output.csv from runtime output",
    ...
    def _warn_if_invalid_csv(self, csv_bytes: bytes) -> None:
        try:
            ...
            self.journal.append_entry("output_validation_warning", "Output is not recognized as CSV")

    def pack_result_file(self, extracted_file: bytes | None) -> None:
        """
    ...
    def get_output_data_from_container(self) -> io.BytesIO:
        ...
        self.journal.append_entry("output_copy_started", "Copying output from runtime container")
        ...
        timespan = self.monitor.stop_output_data_copy_time()
        archive_bytes = io.BytesIO(b"".join(archive_stream))
        archive_bytes.seek(0)
        self.journal.append_entry(
            "output_copied",
            "Copied output from runtime container",
            size_bytes=archive_bytes.getbuffer().nbytes,
            duration_seconds=timespan.total_seconds(),
        )
    ...
    def verify_result_data(self, output_df: pd.DataFrame) -> None:
        ...
        if unexpected_predictors:
            ...
            warnings.append(f"Unexpected predictors: {', '.join(unexpected_predictors)}")
        ...
        if input_rows != output_rows:
            ...
            self.job_context.logger.warning(warning)
            warnings.append(warning)

        self.journal.append_entry(
            "output_validated",
            "Validated runtime output against output schema",
            valid=not warnings,
...
def run_inference(
    ...
    runner = InferenceRunner(input_data, job_context, monitor, journal)
    return runner.run()
